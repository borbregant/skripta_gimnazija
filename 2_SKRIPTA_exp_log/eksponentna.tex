...
\begin{zgled}
    \begin{itemize}
    \item Določimo eksponentno funkcijo $f$, katere graf poteka skozi $A(2,9)$. Nato v isti koordnatni sistem narišimo $f(x), f(x+1), f(x+1)-1, f(-x),\\ |f(x)-3|$.
    \end{itemize}
    
\end{zgled}

\begin{example}
Poišči predpis za eksponentno funkcijo $f(x)=a^x$, za katero je:\\
(a) $f\left(\frac{1}{2}\right)=2$\qquad (b) $f\left(-\frac{3}{2}\right)=\frac{27}{125}$ \qquad (c) $f(-2)=\frac{9}{4}$
\end{example}
\begin{example}
...
\end{example}
\begin{example}
Zapiši tri čim lepše točke grafa $f(x)=2^{x-2}-1$. Nato nariši 
 grafe $f(x)$, $g(x)=|f(x)|$ in $h(x)=f(|x|)$.
\end{example}
\begin{example}
...
$a^{f(x)}=b$ & \textit{Logaritmiranje}&$2^{x-3}=16$\\\hline
\end{tabular}
